...
} from '@opentelemetry/instrumentation-aws-lambda/build/src/types';
...
const name = '@opentelemetry/instrumentation-aws-lambda'
...
export class AwsLambdaInstrumentation extends InstrumentationBase {
    ...
    init() {
        ...
        for (let ext of extensions) {
            try {
                ...
                // if we couldn't find that file that is okay/expected since we are checking for a few different extensions
            }
        }
    ...
    setTracerProvider(tracerProvider: TracerProvider) {
        ...
        this._forceFlush = AwsLambdaInstrumentation._getForceFlush(tracerProvider);
    }

    static _getForceFlush(tracerProvider: TracerProvider) {
    ...
    // Until ES Modules are supported with autoinstrumentation
    // users will manually have to instrument the lambda function
    public static TraceLambda(handler, config = {}, traceProvider) {
        ...
        return async function tracedHandler(event, lambdaContext, callback) {
            ...
            // Use the OpenTelemetry context API to set the current span as active
