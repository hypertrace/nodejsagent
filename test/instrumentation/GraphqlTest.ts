...
`;
    ...
    it('can capture graphql span attrs', async () => {
        ...
        expect(graphqlExecuteSpan.name).to.equal('query')
        expect(graphqlExecuteSpan.attributes['graphql.operation.type']).to.equal('query')
    ...
    it('can capture apollo persisted query span attrs', async () => {
        ...
        let resolveSpan = spans[3]
        expect(resolveSpan.name).to.equal('graphql.resolve books')
        ...
        let graphqlExecuteSpan = spans[4]
        expect(graphqlExecuteSpan.name).to.equal('query')
        expect(graphqlExecuteSpan.attributes['graphql.operation.type']).to.equal('query')
